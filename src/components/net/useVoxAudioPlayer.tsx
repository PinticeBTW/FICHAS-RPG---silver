...
import {
  recordNetVoxAudioRecentPlay,
  signNetVoxAudioTrack,
} from '../../lib/netVoxAudioService'
import type { NetVoxAudioTrack } from '../../lib/netVoxAudioTypes'

export type VoxAudioRepeatMode = 'off' | 'track' | 'queue'

export interface VoxAudioPlayerController {
  readonly current: NetVoxAudioTrack | null
  readonly queue: readonly NetVoxAudioTrack[]
  ...
  readonly repeat: VoxAudioRepeatMode
  readonly bindAudioElement: (element: HTMLAudioElement | null) => void
  readonly playQueue: (tracks: readonly NetVoxAudioTrack[], startIndex?: number) => Promise<void>
  readonly playNext: (track: NetVoxAudioTrack) => Promise<void>
  readonly addToQueue: (track: NetVoxAudioTrack) => Promise<void>
...
export function useVoxAudioPlayer(
  identitySessionKey: string,
  expectedIdentityLinkId?: string,
): VoxAudioPlayerController {
  const audioRef = useRef<HTMLAudioElement | null>(null)
  const generationRef = useRef(0)
  const queueRef = useRef<readonly NetVoxAudioTrack[]>([])
  const indexRef = useRef(-1)
  const identityRef = useRef(expectedIdentityLinkId)
  const shuffleRef = useRef(false)
  const repeatRef = useRef<VoxAudioRepeatMode>('off')
  const [queue, setQueue] = useState<readonly NetVoxAudioTrack[]>([])
  ...
  const [repeat, setRepeat] = useState<VoxAudioRepeatMode>('off')
  ...
  const commitQueue = useCallback((nextQueue: readonly NetVoxAudioTrack[], nextIndex = indexRef.current) => {
  ...
  const loadIndex = useCallback(async (nextIndex: number, nextQueue = queueRef.current) => {
    ...
    setError(undefined)
    try {
      const signedUrl = await signNetVoxAudioTrack(track)
      ...
      if (generationRef.current !== generation || identityRef.current !== identityLinkId) return
      setPlaying(true)
      void recordNetVoxAudioRecentPlay(identityLinkId, track.id).catch(() => undefined)
    } catch (loadError) {
  ...
  const playQueue = useCallback(async (
    tracks: readonly NetVoxAudioTrack[],
    startIndex = 0,
  ) => {
  ...
  const playNext = useCallback(async (track: NetVoxAudioTrack) => {
  ...
  const addToQueue = useCallback(async (track: NetVoxAudioTrack) => {
    const activeQueue = [...queueRef.current]
    let currentIndex = indexRef.current
    const currentTrack = currentIndex >= 0 ? activeQueue[currentIndex] : undefined
    if (!activeQueue.length || currentIndex < 0 || !currentTrack) {
