import type {
  ...
  NetVeilConversationMutationResult,
  NetVeilConversationSummary,
  NetVeilDeleteGroupResult,
  NetVeilLatestMessage,
  NetVeilLeaveGroupResult,
  NetVeilMessage,
  NetVeilMessageCursor,
  NetVeilMessagePage,
  NetVeilMessengerIdentity,
  NetVeilMessengerSidebar,
  NetVeilReadCursorResult,
...
function record(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`RELAY returned an invalid ${label}.`)
  }
...
function timestamp(row: Record<string, unknown>, key: string, label: string): string {
  ...
  if (Number.isNaN(Date.parse(value))) throw new Error(`RELAY returned an invalid ${label}.`)
  return value
}
...
function parseMember(value: unknown): NetVeilConversationMember {
  const row = record(value, 'conversation member')
  const role = row.role
  if (role !== 'owner' && role !== 'member') throw new Error('RELAY returned an invalid member role.')
  if (typeof row.available !== 'boolean') throw new Error('RELAY returned an invalid member state.')
  return { identity: parseIdentity(row.identity), role, available: row.available }
}

function parseMembers(value: unknown): readonly NetVeilConversationMember[] {
  if (!Array.isArray(value)) throw new Error('RELAY returned invalid conversation members.')
  return value.map(parseMember)
}

function parseLatestMessage(value: unknown): NetVeilLatestMessage | undefined {
  if (value === null || value === undefined) return undefined
  const row = record(value, 'latest message')
  if (typeof row.mine !== 'boolean') throw new Error('RELAY returned an invalid latest-message state.')
  return {
...
function parseSummary(value: unknown): NetVeilConversationSummary {
  const row = record(value, 'conversation')
  const kind = row.kind
  const role = row.role
  if (kind !== 'direct' && kind !== 'group') throw new Error('RELAY returned an invalid conversation kind.')
  if (role !== 'owner' && role !== 'member') throw new Error('RELAY returned an invalid conversation role.')
  if (typeof row.can_send !== 'boolean' || typeof row.unread_capped !== 'boolean') {
    throw new Error('RELAY returned an invalid conversation state.')
  }
...
function parseSidebar(value: unknown): NetVeilMessengerSidebar {
  const row = record(value, 'sidebar payload')
  if (!Array.isArray(row.conversations)) throw new Error('RELAY returned an invalid conversation list.')
  ...
  if (row.status !== 'ready') throw new Error('RELAY returned an unsupported session status.')
  return {
    status: 'ready',
...
function parseMessage(value: unknown): NetVeilMessage {
  const row = record(value, 'message')
  if (typeof row.mine !== 'boolean') throw new Error('RELAY returned an invalid message state.')
  return {
...
function parseDetail(value: unknown): NetVeilConversationDetail {
  const row = record(value, 'conversation detail')
  const kind = row.kind
  const role = row.role
  if (kind !== 'direct' && kind !== 'group') throw new Error('RELAY returned an invalid conversation kind.')
  if (role !== 'owner' && role !== 'member') throw new Error('RELAY returned an invalid conversation role.')
  if (typeof row.can_send !== 'boolean') throw new Error('RELAY returned an invalid send state.')
  const avatarUrl = optionalString(row, 'avatar_url')
  return {
    conversationId: requiredString(row, 'conversation_id', 'conversation id'),
    kind,
...
function parseCursor(value: unknown): NetVeilMessageCursor | undefined {
  if (value === null || value === undefined) return undefined
...
function parseMessagePage(value: unknown): NetVeilMessagePage {
  const row = record(value, 'message page')
  if (!Array.isArray(row.messages)) throw new Error('RELAY returned an invalid message page.')
...
function parseMutation(value: unknown): NetVeilConversationMutationResult {
...
async function rpc<T>(name: string, args: Record<string, unknown>, parse: (value: unknown) => T): Promise<T> {
  ...
  if (error) throw new Error(`RELAY request failed: ${error.message}`)
  // Every RELAY RPC returns one jsonb value. A one-result directory search
  // is itself a JSON array and must never be unwrapped here.
  return resolveMessengerAvatarReferences(parse(data))
}

export function fetchNetVeilMessengerSidebar(expectedIdentityLinkId: string, limit = 50) {
  return rpc('fetch_net_veil_messenger_sidebar', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_limit: limit,
  }, parseSidebar)
}

export function searchNetVeilMessengerRecipients(expectedIdentityLinkId: string, query: string, limit = 20) {
  return rpc('search_net_veil_messenger_recipients', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_query: query,
    requested_limit: limit,
  }, (value) => {
    if (!Array.isArray(value)) throw new Error('RELAY returned invalid recipient results.')
    return value.map(parseIdentity)
  })
}

export function ensureNetVeilDirectConversation(expectedIdentityLinkId: string, recipientIdentityLinkId: string) {
  return rpc('ensure_net_veil_direct_conversation', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_recipient_identity_link_id: recipientIdentityLinkId,
  }, parseMutation)
}

export function createNetVeilGroup(expectedIdentityLinkId: string, title: string, memberIdentityLinkIds: readonly string[]) {
  return rpc('create_net_veil_group', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
...
export function renameNetVeilGroup(expectedIdentityLinkId: string, conversationId: string, title: string) {
  return rpc('rename_net_veil_group', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
...
export function addNetVeilGroupMembers(expectedIdentityLinkId: string, conversationId: string, memberIdentityLinkIds: readonly string[]) {
  return rpc('add_net_veil_group_members', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
    requested_member_identity_link_ids: [...memberIdentityLinkIds],
  }, parseMutation)
...
export function removeNetVeilGroupMember(expectedIdentityLinkId: string, conversationId: string, memberIdentityLinkId: string) {
  return rpc('remove_net_veil_group_member', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
...
export function leaveNetVeilGroup(expectedIdentityLinkId: string, conversationId: string): Promise<NetVeilLeaveGroupResult> {
  return rpc('leave_net_veil_group', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
  }, (value) => {
    const row = record(value, 'leave result')
    if (typeof row.left !== 'boolean') throw new Error('RELAY returned an invalid leave state.')
    return {
      conversationId: requiredString(row, 'conversation_id', 'conversation id'),
      left: row.left,
    }
...
export function deleteNetVeilGroup(expectedIdentityLinkId: string, conversationId: string): Promise<NetVeilDeleteGroupResult> {
  return rpc('delete_net_veil_group', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
  }, (value) => {
    const row = record(value, 'delete result')
    if (typeof row.deleted !== 'boolean') throw new Error('RELAY returned an invalid delete state.')
    return {
      conversationId: requiredString(row, 'conversation_id', 'conversation id'),
      deleted: row.deleted,
    }
...
export function fetchNetVeilMessagePage(
  expectedIdentityLinkId: string,
  conversationId: string,
  cursor?: NetVeilMessageCursor,
  limit = 30,
) {
  return rpc('fetch_net_veil_message_page', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
...
export function sendNetVeilMessage(
  expectedIdentityLinkId: string,
  conversationId: string,
  body: string,
  requestKey: string,
): Promise<NetVeilSendMessageResult> {
  return rpc('send_net_veil_message', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
    requested_body: body,
    ...
    if (typeof row.created !== 'boolean') throw new Error('RELAY returned an invalid idempotency state.')
    return { ...message, created: row.created }
  })
}

export function markNetVeilConversationRead(
  expectedIdentityLinkId: string,
  conversationId: string,
  observedMessageId: string,
): Promise<NetVeilReadCursorResult> {
  return rpc('mark_net_veil_conversation_read', {
    requested_expected_identity_link_id: expectedIdentityLinkId,
    requested_conversation_id: conversationId,
    ...
    if (typeof row.updated !== 'boolean') {
      throw new Error('RELAY returned an invalid read-cursor state.')
    }
