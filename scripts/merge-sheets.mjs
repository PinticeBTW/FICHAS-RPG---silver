/**
 * Junta as paginas separadas em PDFs combinados para o site.
 *
 * COMO USAR:
 ...
 *   pag3ram-blue.pdf   pag3ram-grey.pdf   pag3ram-red.pdf
 *   pag3pe-blue.pdf    pag3pe-grey.pdf    pag3pe-red.pdf
...
for (const color of colors) {
  for (const sex of sexes) {
    ...
    // Paginas 1 e 2 (iguais para M e F)
    ...
    // Pagina 3: Codex RAM
    const p3ramBytes = readFileSync(join(SRC, `pag3ram-${color}.pdf`))
    const p3ramDoc = await PDFDocument.load(p3ramBytes)
    const [p3ramCopied] = await merged.copyPages(p3ramDoc, [0])
    merged.addPage(p3ramCopied)

    // Pagina 4: Codex PE
    const p3peBytes = readFileSync(join(SRC, `pag3pe-${color}.pdf`))
    const p3peDoc = await PDFDocument.load(p3peBytes)
    const [p3peCopied] = await merged.copyPages(p3peDoc, [0])
    merged.addPage(p3peCopied)

    // Pagina 5: corpo horizontal (M ou F)
